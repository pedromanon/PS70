                    ...
                    case '/Projects':
                        switch (value) {
                            ...
                            case "Outputs.txt":
                                ...
                                var oscilloscopeexplain = document.createElement('p');
                                oscilloscopeexplain.textContent = "I decided to take readings from the system by using an oscilloscope and measuring the B-1A data pin that would control the motor. Below you can see a video of my results before and after scanning my ID to activate the motor."
                                outputsDiv.appendChild(oscilloscopeexplain);

                                outputsDiv.appendChild(document.createElement('br'));

                                const oscilloscopevid = document.createElement('video');
                                oscilloscopevid.classList.add('firstdraft-img');
                                oscilloscopevid.src = 'Week7/Oscilloscope.MOV';
                                oscilloscopevid.type = 'video/quicktime';
                                oscilloscopevid.controls = true;
                                oscilloscopevid.alt = 'Video of oscilloscope readings';
                                outputsDiv.appendChild(oscilloscopevid);

                                outputsDiv.appendChild(document.createElement('br'));

                                var oscilloscoperesults = document.createElement('p');
                                oscilloscoperesults.textContent = "The oscilloscope readings of my system were a bit strange, but they made sense. For example, rather than perfectly showing a low voltage of 0 and a high voltage of 5, whenever the voltage in my system was low, it would oscillate around 0 with a peak at 272 mV, and a low of -272 mV. There was also a similar oscillation at the 5V mark. Disregarding these oscillations by zooming out like I did in the video, the results are what they should be since the voltage goes up to around 5V when I scan my ID and stays in this high voltage for just 10 seconds. The voltage then goes down because we change the direction of the motor, so the data pin we are measuring drops to a lower voltage to do so. The period of the oscillations were microseconds long, but disregarding the strange oscillations, the period between low and high voltage in the system was only the 10 seconds I programmed it to be."
                                outputsDiv.appendChild(oscilloscoperesults);
                            ...
                            case "Networking.txt":
                                // Create a new div for the content
                                var networkingDiv = document.createElement('div');
                                networkingDiv.classList.add('networking-div');

                                // Create paragraphs for the statements
                                var networkingintro = document.createElement('p');
                                networkingintro.textContent = "The assignment for this week had me work with Alyssa Suh and Yaying Liang Li. The details for the assingment can be found in Alyssa's website:"
                                networkingDiv.appendChild(networkingintro);

                                var networkingwrapper = document.createElement('div');
                                var networkingLink = document.createElement('a');
                                networkingLink.href = 'https://alyssasuh.github.io/PS70/09_networking/index.html';
                                networkingLink.textContent = 'here';
                                networkingwrapper.appendChild(networkingLink);
                                networkingDiv.appendChild(networkingwrapper);

                                networkingDiv.appendChild(document.createElement('br'));
